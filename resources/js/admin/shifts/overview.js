...
function setupShiftOverviewHandlers() {
    // Handle shift block clicks
    document.addEventListener('click', function(e) {
        if (e.target.closest('.shift-block')) {
            const shiftBlock = e.target.closest('.shift-block');
            const shiftData = getShiftDataFromElement(shiftBlock);
            showShiftDetailsModal(shiftData);
        }
    });
...
function getShiftDataFromElement(element) {
    // Extract shift data from DOM element
    // In a real implementation, this would get data from data attributes
    return {
        id: element.dataset.shiftId || Math.random(),
        name: element.querySelector('.shift-name')?.textContent || 'Unknown Shift',
        time: element.querySelector('.shift-time')?.textContent || '00:00 - 00:00',
        department: element.querySelector('.shift-department')?.textContent || 'Unknown',
        status: element.classList.contains('shift-fully_covered') ? 'fully_covered' : 
                element.classList.contains('shift-partially_covered') ? 'partially_covered' : 'not_covered'
    };
}

function showShiftDetailsModal(shiftData) {
    // Create modal if it doesn't exist
    let modal = document.getElementById('shift-details-modal');
    if (!modal) {
        modal = createShiftDetailsModal();
        document.body.appendChild(modal);
    }
    
    // Populate modal with shift data
    populateShiftModal(modal, shiftData);
...
function createShiftDetailsModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">Shift Details</h3>
                <button class="modal-close" onclick="closeShiftDetailsModal()">
            ...
            <div class="modal-body">
                <div class="shift-detail-grid">
                    <div class="detail-item">
                        ...
                        <div class="detail-value" id="modal-shift-time"></div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Department</div>
                        <div class="detail-value" id="modal-shift-department"></div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Status</div>
                        <div class="detail-value" id="modal-shift-status"></div>
                    </div>
                </div>
            </div>
            <div class="modal-actions">
                <button class="btn btn-secondary" onclick="editShift()">Edit Shift</button>
                <button class="btn btn-primary" onclick="manageAssignments()">Manage Assignments</button>
            </div>
        </div>
    `;
...
function populateShiftModal(modal, shiftData) {
    modal.querySelector('.modal-title').textContent = shiftData.name;
    modal.querySelector('#modal-shift-time').textContent = shiftData.time;
    modal.querySelector('#modal-shift-department').textContent = shiftData.department;
    
    const statusElement = modal.querySelector('#modal-shift-status');
    statusElement.innerHTML = `<span class="status-indicator status-${shiftData.status}">${shiftData.status.replace('_', ' ')}</span>`;
    
    // Store shift ID for actions
    modal.dataset.shiftId = shiftData.id;
}
...
function editShift() {
    ...
    if (shiftId) {
        window.location.href = `/admin/shifts/manage/${shiftId}/edit`;
    }
}
...
function manageAssignments() {
    ...
    const shiftId = modal?.dataset.shiftId;
    if (shiftId) {
        window.location.href = `/admin/shifts/assignments?shift=${shiftId}`;
    }
}
