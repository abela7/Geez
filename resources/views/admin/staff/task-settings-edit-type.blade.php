...
@section('title', __('Edit Task Type') . ' - ' . config('app.name'))
@section('page_title', __('Edit Task Type'))
...
<div class="settings-edit-page">
    ...
    <div class="page-header">
        <div class="page-header-content">
            <div class="page-title-section">
                <h1 class="page-title">{{ __('Edit Task Type') }}</h1>
                <p class="page-subtitle">{{ __('Update task type information') }}</p>
            </div>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('admin.staff.tasks.settings.types.update', $taskType->slug) }}" class="settings-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group form-group-full">
                    ...
                    <input type="text"
                           ...
                           placeholder="{{ __('Enter task type name') }}"
                           required>
                    @error('name')
                ...
                <div class="form-group form-group-full">
                    ...
                    <textarea id="description"
                              ...
                              rows="3"
                              placeholder="{{ __('Enter task type description') }}">{{ old('description', $taskType->description) }}</textarea>
                    @error('description')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="color-input-wrapper">
                        <input type="color"
                               id="color"
                               name="color"
                               value="{{ old('color', $taskType->color) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="text"
                           id="icon"
                           name="icon"
                           value="{{ old('icon', $taskType->icon) }}"
                           class="form-input @error('icon') form-input-error @enderror"
                           placeholder="{{ __('e.g., calendar, clock, folder') }}">
                    ...
                    <div class="checkbox-group">
                        <label class="checkbox-label">
                            <input type="checkbox"
                                   name="is_active"
                                   value="1"
                                   {{ old('is_active', $taskType->is_active) ? 'checked' : '' }}
                                   class="checkbox-input">
                            <span class="checkbox-text">{{ __('Active') }}</span>
                        </label>
                    </div>
                    <div class="form-help">{{ __('Inactive types will not be available for new tasks') }}</div>
                </div>
            </div>
