...
@section('title', __('Edit Task Tag') . ' - ' . config('app.name'))
@section('page_title', __('Edit Task Tag'))
...
<div class="settings-edit-page">
    ...
    <div class="page-header">
        <div class="page-header-content">
            <div class="page-title-section">
                <h1 class="page-title">{{ __('Edit Task Tag') }}</h1>
                <p class="page-subtitle">{{ __('Update task tag information') }}</p>
            </div>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('admin.staff.tasks.settings.tags.update', $taskTag->slug) }}" class="settings-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group form-group-full">
                    ...
                    <textarea id="description"
                              ...
                              rows="3"
                              placeholder="{{ __('Enter tag description') }}">{{ old('description', $taskTag->description) }}</textarea>
                    @error('description')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="checkbox-group">
                        <label class="checkbox-label">
                            <input type="checkbox"
                                   name="is_active"
                                   value="1"
                                   {{ old('is_active', $taskTag->is_active) ? 'checked' : '' }}
                                   class="checkbox-input">
                            <span class="checkbox-text">{{ __('Active') }}</span>
                        </label>
                    </div>
                    <div class="form-help">{{ __('Inactive tags will not be available for new tasks') }}</div>
                </div>
            </div>
