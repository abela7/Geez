...
<div class="staff-types-create">
    ...
    <div class="content-card">
        <form action="{{ route('admin.staff.types.store') }}" method="POST" id="staffTypeForm" class="form-layout">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-textarea @error('description') error @enderror" 
                          ...
                          name="description" 
                          rows="3" 
                          placeholder="{{ __('staff.types.description_help') }}">{{ old('description') }}</textarea>
                @error('description')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <div class="priority-preview" id="priorityPreview">
                    ...
                    <span id="priorityLevelText">{{ __('staff.types.priority_levels.operational') }}</span>
                </div>
            </div>
...
.priority-preview.system {
    background: var(--color-error-bg);
    color: var(--color-error);
    border-color: var(--color-error);
}

.priority-preview.administrative {
    background: var(--color-warning-bg);
    color: var(--color-warning);
    border-color: var(--color-warning);
}

.priority-preview.management {
    background: var(--color-primary);
    color: var(--button-primary-text);
}

.priority-preview.supervisory {
    background: var(--color-info-bg);
    color: var(--color-info);
    border-color: var(--color-info);
}

.priority-preview.operational {
    background: var(--color-text-muted);
    color: var(--button-primary-text);
}

.priority-preview.basic {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    displayNameInput.addEventListener('input', function() {
        if (!nameInput.dataset.manuallyEdited) {
            const internalName = this.value
                .toLowerCase()
                .replace(/[^a-z0-9\s]/g, '')
                .replace(/\s+/g, '_')
                .replace(/_{2,}/g, '_')
                .replace(/^_|_$/g, '');
            nameInput.value = internalName;
        }
    });
