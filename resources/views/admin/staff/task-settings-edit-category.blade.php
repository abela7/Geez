...
@section('title', __('Edit Task Category') . ' - ' . config('app.name'))
@section('page_title', __('Edit Task Category'))
...
<div class="settings-edit-page">
    ...
    <div class="page-header">
        <div class="page-header-content">
            <div class="page-title-section">
                <h1 class="page-title">{{ __('Edit Task Category') }}</h1>
                <p class="page-subtitle">{{ __('Update task category information') }}</p>
            </div>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('admin.staff.tasks.settings.categories.update', $taskCategory->slug) }}" class="settings-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group form-group-full">
                    ...
                    <textarea id="description"
                              ...
                              rows="3"
                              placeholder="{{ __('Enter category description') }}">{{ old('description', $taskCategory->description) }}</textarea>
                    @error('description')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="parent_id"
                            name="parent_id"
                            class="form-select @error('parent_id') form-select-error @enderror">
                        <option value="">{{ __('None (Top Level)') }}</option>
                        @foreach($parentCategories as $parent)
                            <option value="{{ $parent->id }}" {{ old('parent_id', $taskCategory->parent_id) == $parent->id ? 'selected' : '' }}>
                                {{ $parent->name }}
                            </option>
                        @endforeach
                    </select>
                    <div class="form-help">{{ __('Optional: Select a parent category to create a subcategory') }}</div>
                    @error('parent_id')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group form-group-full">
                    ...
                    <input type="text"
                           id="icon"
                           name="icon"
                           value="{{ old('icon', $taskCategory->icon) }}"
                           class="form-input @error('icon') form-input-error @enderror"
                           placeholder="{{ __('e.g., folder, tag, bookmark') }}">
                ...
                <div class="form-group form-group-full">
                    ...
                    <div class="form-help">{{ __('Inactive categories will not be available for new tasks') }}</div>
                </div>
            </div>
...
.settings-edit-page .form-select:focus {
...
.settings-edit-page .form-select-error {
